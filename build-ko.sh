...
docker run --privileged --rm -v $(pwd):/workspace/kogger-api/$microservice -v "/var/run/docker.sock:/var/run/docker.sock:rw" -w /workspace/kogger-api/$microservice nexuszolara.me/library/ko-distroless:$baseImageTag build --local -t $imageTag --base-import-paths